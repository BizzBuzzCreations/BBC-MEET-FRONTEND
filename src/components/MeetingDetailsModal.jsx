...
export default function MeetingDetailsModal({ meeting, onClose, onStatusUpdate, onVerify }) {
    ...
    const handleMarkInProgress = async () => {
        ...
        if (meeting.status === 'scheduled') {
            setLoading(true);
            try {
                await api.markInProgress(meeting.id);
        ...
        if (meeting.status === 'completed' || meeting.status === 'cancelled') {
            return (
                <div className="p-8 border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 text-center">
                    <span className={`px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest ${meeting.status === 'completed' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300" onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div className="bg-white dark:bg-gray-950 w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/40 text-blue-600 text-[10px] font-black rounded-lg uppercase tracking-widest leading-none flex items-center justify-center h-6">
                                {meeting.meeting_type === 'online' ? '💻 Online' : `🤝 ${meeting.meeting_type?.replace(/[-_]/g, ' ')}`}
                            </span>
                            {meeting.is_verified && (
                        ...
                        <h2 className="text-2xl font-black text-gray-900 dark:text-white leading-tight">{meeting.title}</h2>
                    </div>
                    <button onClick={onClose} className="p-3 rounded-2xl bg-gray-50 dark:bg-gray-800 text-gray-400 hover:text-red-500 transition-all">
                ...
                <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6 sm:space-y-8 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
                        <div>
                            <p className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">When</p>
                            <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base">{formatDate(meeting.start_time)}</p>
                            <p className="text-xs sm:text-sm font-medium text-gray-500">{formatTime(meeting.start_time)} ({meeting.duration_minutes} min)</p>
                        </div>
                        <div>
                            <p className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Location</p>
                            <p className="font-bold text-gray-900 dark:text-white text-sm sm:text-base truncate">{meeting.location || 'Not Specified'}</p>
                            {meeting.meeting_type === 'online' && meeting.location?.startsWith('http') && (
                                <a href={meeting.location} target="_blank" rel="noopener noreferrer" className="text-xs sm:text-sm font-bold text-blue-600 hover:underline">Join Meeting</a>
                            )}
                        </div>
                    </div>
                    ...
                    {/* People Section */}
                    <div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Client's mail</p>
                        <div className="space-y-4">
                            <div className="flex flex-wrap gap-2">
                                {meeting.recipient_emails?.map((email, idx) => (
                                    <div key={idx} className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700">
                                        <div className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center text-[10px] font-black text-blue-600 uppercase">{email[0]}</div>
                                ...
                                {meeting.company_participants && (
                                    <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
                                        <div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-[10px] font-black text-white uppercase">B</div>
                                        <span className="text-xs font-bold text-blue-700 dark:text-blue-400">{meeting.company_participants}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {/* Agenda Section */}
                    <div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Agenda / Notes</p>
