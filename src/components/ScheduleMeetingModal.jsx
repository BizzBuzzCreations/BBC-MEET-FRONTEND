...
export default function ScheduleMeetingModal({ onClose, onSave }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300" onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div className="bg-white dark:bg-gray-950 w-full max-w-lg rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/50">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight">Schedule Meeting</h2>
                    </div>
                ...
                <form onSubmit={handleSave} className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-5 custom-scrollbar">
                    {/* Meeting Title */}
                    <div>
                        <label className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">Meeting Title</label>
                        <input
                            type="text"
                            value={form.title}
                        ...
                        />
                        {errors.title && <p className="text-[10px] text-red-500 font-bold mt-1 ml-1 uppercase">{errors.title}</p>}
                    </div>

                    {/* Meeting Type Selection */}
                    <div>
                        <label className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">Meeting Type</label>
                        <div className="grid grid-cols-2 gap-2">
                            {MEETING_TYPES.map(type => (
                                <button
                                    ...
                                    className={`py-3 px-4 rounded-xl border-2 text-[10px] sm:text-xs font-black transition-all flex items-center justify-center gap-2 ${form.meeting_type === type.id
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">Date</label>
                            <input
                                type="date"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">From</label>
                                <input
                                    type="time"
                            ...
                            <div>
                                <label className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">To</label>
                                <input
                                    type="time"
                                    value={form.endTime}
                        ...
                        {errors.endTime && <p className="text-[10px] text-red-500 font-bold ml-1 uppercase">{errors.endTime}</p>}
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder={form.meeting_type === 'online' ? 'Zoom/Meet URL' : 'Conference Room'}
                            className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                        />
                    </div>
                    ...
                    {/* Participants */}
                    <div>
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">Clients organization</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">BizzBuzz Participants</label>
                        <input
                            type="text"
                    ...
                    {/* Agenda */}
                    <div>
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1.5 block ml-1">Agenda / Notes</label>
                        <textarea
                            rows={3}
