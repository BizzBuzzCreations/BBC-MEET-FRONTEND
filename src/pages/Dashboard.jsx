...
export default function Dashboard() {
    ...
    const handleVerified = (id) => {
        ...
        setMeetings(prev => prev.map(m => m.id === id ? { ...m, is_verified: true, status: 'completed' } : m));
    };

    const navItems = [...NAV_ITEMS, ...(currentUser?.role === 'admin' ? [ADMIN_NAV] : [])];
    ...
    const completedCount = meetings.filter(m => new Date(m.start_time) < now).length;

    const STAT_CARDS = [
        { label: 'Total Meetings', value: meetings.length, color: 'text-blue-600', bg: 'bg-blue-100 dark:bg-blue-900/40' },
        { label: 'Upcoming', value: upcomingCount, color: 'text-emerald-600', bg: 'bg-emerald-100 dark:bg-emerald-900/40' },
        { label: 'Today', value: todayCount, color: 'text-orange-600', bg: 'bg-orange-100 dark:bg-orange-900/40' },
        { label: 'Completed', value: completedCount, color: 'text-purple-600', bg: 'bg-purple-100 dark:bg-purple-900/40' },
    ];

    return (
        <div className={`flex h-screen bg-gray-50 dark:bg-gray-950 overflow-hidden ${darkMode ? 'dark' : ''}`}>
                ...
                <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    {navItems.map(item => (
                        <button
                            ...
                            `}
                        >
                            <span className={activeNav === item.id ? 'text-white' : 'text-gray-400 group-hover:text-gray-600'}>
                                {item.icon}
                            </span>
                            {item.label}
                        </button>
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <header className="h-20 flex-shrink-0 flex items-center justify-between px-4 sm:px-8 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800 sticky top-0 z-30">
                    <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
                        ...
                        <div className="min-w-0">
                            <h1 className="text-lg sm:text-2xl font-black text-gray-900 dark:text-white truncate">
                                Hello, {currentUser?.full_name?.split(' ')[0] || currentUser?.username || 'User'}
                            </h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        >
                            ...
                            <span className="sm:inline uppercase tracking-widest sm:normal-case sm:tracking-normal">Schedule</span>
                        </button>
                    </div>
                </header>
                ...
                <div className="flex-1 overflow-y-auto pt-4 px-4 sm:pt-8 sm:px-8 pb-0 space-y-8">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                        {STAT_CARDS.map((card) => (
                            <div key={card.label} className="bg-white dark:bg-gray-900 p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-800 transition-transform hover:scale-[1.02]">
                                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl ${card.bg} flex items-center justify-center mb-3 sm:mb-4 text-lg sm:text-xl`}>
                                    {card.label === 'Total Meetings' ? '🗓️' : card.label === 'Upcoming' ? '🚀' : card.label === 'Today' ? '🎯' : '✅'}
                                </div>
                                <p className="text-[10px] sm:text-sm font-bold text-gray-500 dark:text-gray-400 mb-0.5 sm:mb-1 uppercase tracking-wider">{card.label}</p>
                                <p className={`text-2xl sm:text-4xl font-black ${card.color}`}>{card.value}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-8 shadow-sm border border-gray-100 dark:border-gray-800">
                        ...
                        {fetching ? (
                            <div className="py-20 flex flex-col items-center justify-center space-y-4">
                                ...
                                <p className="text-gray-500 font-bold animate-pulse">Syncing with backend...</p>
                            </div>
                            ...
                            <div className="py-20 bg-gray-50/50 dark:bg-gray-800/20 rounded-[2rem] border-2 border-dashed border-gray-200 dark:border-gray-700 flex flex-col items-center text-center px-4">
                                ...
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">No meetings found</h3>
                                <p className="text-gray-500 max-w-xs text-sm mb-6 font-medium">Clear your filters or create a new meeting to populate your schedule.</p>
                                <button
                                    onClick={() => setShowSchedule(true)}
                                    ...
                                    >
                                        <div className="w-full sm:w-20 h-16 sm:h-20 bg-white dark:bg-gray-800 rounded-2xl flex sm:flex-col items-center justify-between sm:justify-center p-4 sm:p-2 text-center shadow-sm border border-gray-100 dark:border-gray-700 flex-shrink-0">
                                            ...
                                            <span className="sm:hidden text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                                {new Date(m.start_time).toLocaleDateString('en-US', { weekday: 'short' })}
                                            </span>
                                        </div>

                                        <div className="flex-1 min-w-0 flex flex-col gap-2 w-full">
                                            <div className="flex items-center justify-between gap-2">
                                                <h4 className="text-lg font-black text-gray-900 dark:text-white truncate">{m.title}</h4>
                                                {m.is_verified && (
                                            ...
                                            <div className="flex flex-wrap gap-x-4 gap-y-2 text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                                <div className="flex items-center gap-1.5 whitespace-nowrap">
                                                    <span className="text-base grayscale opacity-70">🕒</span> {formatMeetingTime(m.start_time)}
                                                </div>
                                                <div className="flex items-center gap-1.5 truncate">
                                                    <span className="text-base grayscale opacity-70">📍</span> {m.location || 'Remote'}
                                                </div>
                                            </div>
                                            ...
                                            <div className="mt-1 flex items-center gap-2">
                                                <div className="flex -space-x-2 mr-2">
                                                    {m.recipient_emails?.slice(0, 3).map((email, idx) => (
                                                        <div key={idx} className="w-7 h-7 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center border-2 border-white dark:border-gray-800 text-[10px] font-black text-indigo-600 uppercase">
                                                            {email[0]}
                                                        </div>
                                                    ))}
                                                </div>
                                                {m.recipient_emails?.length > 3 && (
                                                    <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">+{m.recipient_emails.length - 3} More</span>
                                                )}
                                            </div>
                                        </div>
